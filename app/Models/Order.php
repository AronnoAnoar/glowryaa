...
class Order extends Model
{
  ...
  protected $fillable = ['id', 'invoice_no', 'order_type', 'customer_id', 'customer_phone', 'city_id', 'sub_city_id', 'courier_id', 'note', 'shipping_cost', 'discount', 'paid', 'paid_by', 'total', 'status', 'print_status', 'memo_no', 'create_admin_id', 'pending_admin_id', 'pending_date', 'approved_admin_id', 'approved_date', 'shipment_admin_id', 'shippment_date', 'delivered_admin_id', 'delivery_date', 'cancel_admin_id', 'cancel_date', 'return_admin_id', 'return_date', 'reseller_id', 'comment', 'coupon_id', 'coupon_disocunt', 'note', 'redx_delivery_area_id', 'created_at', 'updated_at'];
    ...
    if ($request->status != "all") {
      if ($request->type == "all") {
        $orders = Order::orderBy('id', 'DESC')
          ->with(['customer:id,name,address', 'createAdmin:id,name', 'approvedBy:id,name', 'courier', 'reseller:id,phone,name', 'orderItem.product:id,name,thumbnail_img,product_code'])
      ...
      if ($request->type == "all") {
        ...
        $orders = Order::orderBy('id', 'DESC')->with(['customer:id,name,address', 'approvedBy:id,name', 'createAdmin:id,name', 'courier', 'reseller:id,phone,name', 'orderItem.product:id,name,thumbnail_img,product_code'])->where('order_type', $request->type)->paginate($paginate);
      }
    }
    return \response()->json([
      'status' => 'SUCCESS',
      'orders' => $orders,
      'order_count' => self::orderStatusCount(),
  ...
  public static function orderFilterWithStartDate($request)
  {
    ...
    if ($request->status != "all") {
      ...
      if ($request->type == "all") {
        ...
        $orders = Order::orderBy('id', 'DESC')->with(['customer:id,name,address', 'createAdmin:id,name', 'courier', 'approvedBy:id,name', 'reseller:id,phone,name', 'orderItem.product:id,name,thumbnail_img,product_code'])
          ->where('order_type', $request->type)
          ->whereDate('created_at', '=', $request->start_date)
          ->paginate($paginate);
      }
  ...
  public static function orderFilterWithCity($request)
  {
    ...
    if ($request->status != "all") {
      if ($request->type == "all") {
        if ($request->city == 1) {
          ...
          $orders = Order::orderBy('id', 'DESC')
            ->with(['customer:id,name,address,total_order,status', 'createAdmin:id,name', 'approvedBy:id,name', 'courier', 'orderItem.product:id,name,thumbnail_img,product_code'])
            ->where('status', $request->status)
            ->paginate($paginate);
        }
      }
    ...
    if ($request->status != "all") {
      if ($request->type == "all") {
        ...
        if ($request->status == 4) {
          $orders = Order::orderBy('id', 'DESC')
            ->with(['customer:id,name,address', 'createAdmin:id,name', 'approvedBy:id,name', 'courier', 'orderItem.product:id,name,thumbnail_img,product_code'])
            ->whereDate('shippment_date', '>=', $request->start_date)
            ->whereDate('shippment_date', '<=', $request->end_date)
            ->paginate($paginate);
        } else {
      ...
      if ($request->type == "all") {
        ...
        $orders = Order::orderBy('id', 'DESC')->with(['customer:id,name,address', 'createAdmin:id,name', 'approvedBy:id,name', 'courier', 'orderItem.product:id,name,thumbnail_img,product_code'])
          ->where('order_type', $request->type)
          ->whereDate('created_at', '>=', $request->start_date)
          ->whereDate('created_at', '<=', $request->end_date)
          ->paginate($paginate);
  ...
  public static function orderFilterWithStartAndEndDateAndCity($request)
  {
    $paginate = $request->item ?? 10;
    if ($request->status != "all") {
  ...
  public static function orderFilterWithCourier($request)
  {
    ...
    if (!empty($request->start_date) && empty($request->end_date)) {
      ...
      if ($request->status != "all") {
        $orders = Order::orderBy('id', 'DESC')
          ->with(['customer:id,name,address', 'approvedBy:id,name', 'createAdmin:id,name', 'courier', 'reseller:id,phone,name', 'orderItem.product:id,name,thumbnail_img,product_code'])
          ->where('status', $request->status)
          ->whereDate('created_at', '=', $request->start_date)
          ->where('courier_id', $request->courier_id)
          ->paginate($paginate);
      } else {
        $orders = Order::orderBy('id', 'DESC')->with(['customer:id,name,address', 'approvedBy:id,name', 'createAdmin:id,name', 'courier', 'reseller:id,phone,name', 'orderItem.product:id,name,thumbnail_img,product_code'])
          ->whereDate('created_at', '=', $request->start_date)
          ->where('courier_id', $request->courier_id)
          ->paginate($paginate);
      }
  ...
  public static function orderPressingReport($admin_id, $start_date, $end_date)
  {
    ...
    $data  = [
      ...
      'pending' => Order::where('pending_admin_id', $admin_id)
        ->where('pending_date', '>=', $start_date)
        ->where('pending_date', '<=', $end_date)
        ->count(),

      'approved' => Order::where('approved_admin_id', $admin_id)
        ->where('approved_date', '>=', $start_date)
        ->where('approved_date', '<=', $end_date)
        ->count(),

      'shipment' => Order::where('approved_admin_id', $admin_id)
        ->where('shippment_date', '>=', $start_date)
        ->where('shippment_date', '<=', $end_date)
        ->count(),

      'delivered' => Order::where('approved_admin_id', $admin_id)->where('status', 5)
        ->where('delivery_date', '>=', $start_date)
        ->where('delivery_date', '<=', $end_date)
        ->count(),

      'returned' => Order::where('approved_admin_id', $admin_id)->where('status', 7)
        ->where('return_date', '>=', $start_date)
        ->where('return_date', '<=', $end_date)
        ->count(),

      'canceled' => Order::where('approved_admin_id', $admin_id)->where('status', 6)
        ->where('cancel_date', '>=', $start_date)
        ->where('cancel_date', '<=', $end_date)
        ->count(),
